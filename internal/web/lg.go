...
func (s *Server) runLookingGlass(r *http.Request) LGView {
	...
	case "protocol":
		page, err = s.client.RoutesByProtocolPage(v.Target, offset, limit)
	case "export":
		page, err = s.client.RoutesExportPage(v.Target, offset, limit)
	case "noexport":
		page, err = s.client.RoutesNoExportPage(v.Target, offset, limit)
	}
	v.Ran = true
	if err != nil {
		v.Err = err.Error()
